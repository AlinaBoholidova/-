...
namespace Курсовая
{
    public partial class Rating : Form
    {
        ...
        {
            string select2 = "SELECT Pupil_ID, AVG(CONVERT(DECIMAL(5,2), JournalRecord.avg)), " +
            ...
            string sel = "SELECT AdditionalParticipation.Pupil_ID, SUM(PointsDistribution.Points), SUM(PointsDistribution.Points)*0.3 AS 'Для рейтингу' " +
        ...
        private void createRateButton_Click(object sender, EventArgs e)
        {
            ...
            {
                value.Add(Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value),
            ...
            {
                dataGridView3.Rows.Add(keyValue.Key, keyValue.Value);
            }
            ...
            int hi = Convert.ToInt32(highlow);
            for (int i = hi; i < count - hi; i++)
            {
                Payment payment = new Payment
                {
                    Pupil_ID = value.ElementAt(i).Key,
